...
#		points.append(Vector2(50-randf()*100, 50-randf()*100))
#
...
#  tris.clear()
  ...
  var i = 0
  while(i<points.size()):
    verts.append(Vector2(points[i].x, points[i].y))
    i+=1
  
  # Create triangle indices
  var triang = TriangulatePolygon(verts)
  return triang
...
static func TriangulatePolygon(XZofVertices):
